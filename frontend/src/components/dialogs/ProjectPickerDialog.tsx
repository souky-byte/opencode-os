...
import { ChevronRight, Folder, FolderGit, FolderOpen, Home, X } from "lucide-react";
...
const ProjectPickerDialogComponent = NiceModal.create<ProjectPickerDialogProps>(
	({ allowClose = true }) => {
		...
		const { data: recentProjectsResponse, isLoading: isLoadingRecent } = useGetRecentProjects();
			...
			mutation: {
				onSuccess: (response) => {
					if (response.data.success && response.data.project) {
						...
						setValidationError(response.data.error?.message ?? "Failed to open project");
					}
				},
				onError: () => {
		...
		const removeRecent = useRemoveRecentProject({
			mutation: {
				onSuccess: () => {
					void queryClient.invalidateQueries({ queryKey: getGetRecentProjectsQueryKey() });
				},
			},
		});
		...
		const pathSegments = browseData?.current_path.split("/").filter(Boolean) ?? [];
			...
			<Dialog open={modal.visible} onOpenChange={handleOpenChange}>
				<DialogContent className="max-w-lg" hideCloseButton={!allowClose}>
					<DialogHeader>
						...
						<DialogDescription>Select a Git or Jujutsu repository to open.</DialogDescription>
					</DialogHeader>
							...
							{browseData?.is_vcs_root && (
								<div className="flex items-center justify-between p-2 bg-green-500/10 border border-green-500/30 rounded">
									...
									<Button
										...
										disabled={isPending}
									>
										{isPending ? <Loader className="h-4 w-4" /> : "Open This Project"}
									</Button>
								</div>
							)}
								...
								<ScrollArea className="h-[280px]">
									<div className="space-y-0.5 pr-4">
										{browseData?.parent_path && (
											<button
												type="button"
												onClick={() => handleBrowseFolder(browseData.parent_path!)}
...
export const ProjectPickerDialog = defineModal<ProjectPickerDialogProps, unknown>(
