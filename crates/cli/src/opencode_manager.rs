...
impl OpenCodeManager {
    ...
    pub async fn ensure_running(&mut self) -> Result<()> {
        ...
        if self.health_check().await {
            println!(
                "  {} {}",
                "✓".green().bold(),
                "OpenCode server already running".green()
            );
            return Ok(());
        }
        ...
        println!(
            "  {} {}",
            "⚡".yellow(),
            format!("Starting OpenCode server on port {}...", self.port).white()
        );
    ...
    async fn spawn_server(&mut self) -> Result<()> {
        ...
        let child = Command::new(&binary)
            ...
            .stderr(Stdio::piped())
            .spawn()
            .with_context(|| format!("Failed to start OpenCode server from {:?}", binary))?;
